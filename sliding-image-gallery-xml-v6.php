...
/* start client side functions */
	...
	function slidingimagegalleryxmlv6_filter_content($content) {
		return preg_replace_callback('|\[sliding-image-gallery-xml-v6\s*(xmlUrl="([^"]+)")?\s*(width="([0-9]+)")?\s*(height="([0-9]+)")?\s*\](.*)\[/sliding-image-gallery-xml-v6\]|i', 'slidingimagegalleryxmlv6_get_embed_code', $content);
	}
